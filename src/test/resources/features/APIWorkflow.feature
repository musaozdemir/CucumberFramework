...
Feature: Syntax HRMS API Workflow
  ...
  Scenario: Retrieving created employee
    ...
    And And the retrieved data at "employee" matches the data used to create an employee with employee ID "employee[0].employee_id"
      | emp_firstname | emp_middle_name | emp_lastname | emp_birthday | emp_gender | emp_job_title   | emp_status              |
      | RRobertoo     | J               | CCarloss     | 1980-10-10   | Male       | Cloud Architect | Employee|
